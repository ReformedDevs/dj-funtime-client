...
hashParser location =
  let
    _ = Debug.log "location" (String.dropLeft 1 location.pathname)
  in
...
view model =
  div []
    [ div [ centerStyle "row" ]
        [ a (clickTo (Home)) [ text "Home" ]
        , a (clickTo (Blog 42)) [ text "Cat Facts" ]
        , a (clickTo (Blog 13)) [ text "Alligator Jokes" ]
        , a (clickTo (Blog 26)) [ text "Workout Plan" ]
...
--       (Json.map (\_ -> Navigation.newUrl (toHash page)) Json.value)
--     ] [ text description ]

-- inner click helper

clickTo : Page -> List (Attribute Msg)
clickTo path =
  [ href  (toHash path)
  , onWithOptions
      "click"
